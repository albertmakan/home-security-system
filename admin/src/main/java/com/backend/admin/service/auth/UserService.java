...
public class UserService implements UserDetailsService {
    ...
    public UserService(UserRepository userRepository, RoleService roleService, HouseholdService householdService,
    ...
    public List<User> findAllWithKeywordAndRole(String keyword, String role) {
        String roleString = role == null ? "" : role;
        List<User> users = userRepository.findByUsernameRegexAndRole(keyword);

        final String finalRole = roleString;

        // TODO dont filter by role like this but find how to implement mongo lookup for
        // querying referenced document objects
        return users.stream().filter(user -> user.getRoles().get(0).getName().contains(finalRole))
